...
- [ ] `sync` helper in `bash/aliases.bash`: fetch/prune + rebase current branch onto default (`main`/`master`), using `git rev-parse --abbrev-ref` and `git config --get init.defaultBranch` as fallbacks.
